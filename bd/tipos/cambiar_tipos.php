...
if(isset($_POST["ID_Tipo"]) && !empty($_POST["ID_Tipo"])){
    ...
    if(empty($tipo_err)){
        $sql = "UPDATE tipo SET Nom_Tipo = ? WHERE ID_Tipo = ?";
		
        if($stmt = mysqli_prepare($mysqli, $sql)){
			mysqli_stmt_bind_param($stmt, "si", $param_tipo, $param_id);
    ...
    if(isset($_GET["ID_Tipo"]) && !empty(trim($_GET["ID_Tipo"]))){
        $id =  trim($_GET["ID_Tipo"]);
        
        $sql = "SELECT * FROM tipo WHERE ID_Tipo = ?";
        if($stmt = mysqli_prepare($mysqli, $sql)){
					...
					<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
						...
						<div class="form-row">
							<div class="form-group col-md-4 <?php echo (!empty($tipo_err)) ? 'has-error' : ''; ?>">
								...
								<input type="text" name="tipo" class="form-control" placeholder="Departamento" value="<?php echo $tipo; ?>" required>
							</div>
							<div class="form-group col-md-4">
								<br>
								<input type="hidden" name="ID_Tipo" value="<?php echo $id; ?>"/>
								<input type="submit" class="btn btn-success" value="Guardar">
								<a class="btn btn-danger" href="ver_tipos.php">Cancelar</a>
							</div>
						</div>
					</form>
				</div>
