...
if(isset($_POST["ID_Inm"]) && !empty($_POST["ID_Inm"])){
    ...
    $sql = "UPDATE inmueble SET Estatus_Inm = ? WHERE ID_Inm = ?";
    
    if($stmt = mysqli_prepare($mysqli, $sql)){
        mysqli_stmt_bind_param($stmt, "ss", $param_estat, $param_id);
        
		$param_estat = trim($_POST["Estatus_Inm"]);
        $param_id = trim($_POST["ID_Inm"]);
					...
					<div class="row">
						<div class="col-md-12">
							<div class="page-header">
								<?php echo $error ?>
								<h2><b>Rechazar Publicación</b></h2>
							</div>
							<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
								<div class="alert alert-warning fade in">
									<input type="hidden" name="Estatus_Inm" value="Rechazado"/>
									<input type="hidden" name="ID_Inm" value="<?php echo trim($_GET["ID_Inm"]); ?>"/>
									<p>¿Estás seguro que deseas rechazar la publicación?</p><br>
									<p>
										<input type="submit" value="Si" class="btn btn-danger">
										<a href="ver_bienes.php" class="btn btn-default">No</a>
									</p>
								</div>
							</form>
						</div>
