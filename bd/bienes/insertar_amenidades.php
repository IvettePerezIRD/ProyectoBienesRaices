...
$ame = $val = $ame_err = $val_err = $error = "";

if(isset($_POST["ID_Inm"]) && !empty($_POST["ID_Inm"])){
    
	//validar amenidad
    $input_ame = trim($_POST["cbx_amenidad"]);
    if(empty($input_ame)){
        $ame_err = "Por favor ingrese una amenidad.";
    } else{
        $ame = $input_ame;
    }
	
	//validar valor
    $input_val = trim($_POST["valor"]);
    if(empty($input_val)){
        $val_err = "Por favor ingrese una valor.";
    } else{
        $val = $input_val;
    }
	
    if(empty($ame_err) && empty($val_err)){
		$sql = "INSERT INTO cantidad (ID_Inm_Can, ID_Ame_Can, Valor_Can) VALUES (?,?,?)";
		
        if($stmt = mysqli_prepare($mysqli, $sql)){
            mysqli_stmt_bind_param($stmt, "iss", $param_inm, $param_ame, $param_val);
            
			$param_inm = trim($_POST["ID_Inm"]);
            $param_ame = $ame;
			$param_val = $val;
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                echo $error = "Algo ha salido mal. Por favor, intentelo de nuevo más tarde.";
            }
        }
		...
		<div id='content'>
			...
			<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
				<div class="form-group col-md-12">
					<?php echo $error ?>
					<h2 class="pull-left"><b>Registro de Amenidades</b></h2>
				</div>
				<div class="form-row">
					<div class="form-group col-md-4 <?php echo (!empty($tipo_err)) ? 'has-error' : ''; ?>">
						<label>Tipo de bien</label>
						<select name="cbx_amenidad" id="cbx_amenidad" class="form-control" required>
							<option value="0">Elegir</option>
							<?php
							$query = "SELECT ID_Ame, Nom_Ame FROM amenidad ORDER BY Nom_Ame";
							$resultado=$mysqli->query($query);
							?>
							<?php while($row = $resultado->fetch_assoc()) { ?>
							<option value="<?php echo $row['ID_Ame']; ?>"><?php echo $row['Nom_Ame']; ?></option>
							<?php } ?>
						</select>
					</div>
					<div class="form-group col-md-4 <?php echo (!empty($precio_err)) ? 'has-error' : ''; ?>">
						<label>Cantidad</label>
						<input type="number" name="valor" class="form-control" id="valor" placeholder="4" required>
					</div>
				</div>
				<div class="form-row">
					<div class="form-group col-md-4">
						<br>
						<input type="hidden" name="ID_Inm" value="<?php echo trim($_GET["ID_Inm"]); ?>"/>
