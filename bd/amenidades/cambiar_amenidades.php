...
if(isset($_POST["ID_Ame"]) && !empty($_POST["ID_Ame"])){
	//validar id
	$id = $_POST["ID_Ame"];
	
	//validar amenidad
    $input_ame = trim($_POST["amenidad"]);
    if(empty($input_ame)){
    ...
    if(empty($ciu_err)){
        $sql = "UPDATE amenidad SET Nom_Ame = ? WHERE ID_Ame = ?";
		
        if($stmt = mysqli_prepare($mysqli, $sql)){
			mysqli_stmt_bind_param($stmt, "si", $param_ame, $param_id);
			
            $param_ame = $ame;
			$param_id = $id;

            if(mysqli_stmt_execute($stmt)){
                header("location: ver_amenidades.php");
                exit();
            } else{
    ...
    if(isset($_GET["ID_Ame"]) && !empty(trim($_GET["ID_Ame"]))){
        $id =  trim($_GET["ID_Ame"]);
        
        $sql = "SELECT * FROM amenidad WHERE ID_Ame = ?";
        if($stmt = mysqli_prepare($mysqli, $sql)){
...
<head>
    ...
    <title>LEGACY 9 | AMENIDADES</title>
    <!-- InstanceEndEditable -->
		...
		<section id='sidebar'>
			...
			<!-- InstanceBeginEditable name="activo" -->
				...
				<li class='active launcher'>
					...
					<a href="ver_amenidades.php">Amenidades</a>
				</li>
				<li class='launcher'>
				...
				<div class='col-lg-12'>
					<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
						...
						<div class="form-row">
							...
							<div class="form-group col-md-3 <?php echo (!empty($ame_err)) ? 'has-error' : ''; ?>">
								<input type="text" name="amenidad" class="form-control" placeholder="Baños" value="<?php echo $ame; ?>" required>
							</div>
							<div class="form-group col-md-3">
								<input type="hidden" name="ID_Ame" value="<?php echo $id; ?>"/>
								<input type="submit" class="btn btn-success" value="Guardar">
							</div>
							<div class="form-group col-md-3">
								<a class="btn btn-danger" href="ver_amenidades.php">Cancelar</a>
							</div>
						</div>
					</form>
				</div>
