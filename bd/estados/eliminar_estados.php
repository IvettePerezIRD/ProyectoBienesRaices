...
if(isset($_POST["ID_Est"]) && !empty($_POST["ID_Est"])){
    ...
    $sql = "DELETE FROM estado WHERE ID_Est = ?";
    
    if($stmt = mysqli_prepare($mysqli, $sql)){
        mysqli_stmt_bind_param($stmt, "i", $param_id);
        
        $param_id = trim($_POST["ID_Est"]);
						...
						<div class="col-md-12">
							...
							<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
								<div class="alert alert-danger fade in">
									<input type="hidden" name="ID_Est" value="<?php echo trim($_GET["ID_Est"]); ?>"/>
									<p>¿Estás seguro que deseas borrar el registro?</p><br>
									<p>
										<input type="submit" value="Si" class="btn btn-danger">
										<a href="ver_estados.php" class="btn btn-default">No</a>
									</p>
								</div>
							</form>
						</div>
