...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($est_err)){
        $sql = "INSERT INTO estado (Nom_Est) VALUES (?)";
		
        if($stmt = mysqli_prepare($mysqli, $sql)){
            mysqli_stmt_bind_param($stmt, "s", $param_est);
			...
			<div class='row'>
				<div class='col-lg-12'>
					<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
						...
						<div class="form-row">
							<div class="form-group col-md-3 <?php echo (!empty($est_err)) ? 'has-error' : ''; ?>">
								<label>Nombre del estado</label>
								<input type="text" name="estado" class="form-control" id="inputName4" placeholder="Texas" required>
							</div>
