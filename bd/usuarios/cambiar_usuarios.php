...
if(isset($_POST["ID_Usu"]) && !empty($_POST["ID_Usu"])){
    ...
    if(empty($email_err) && empty($name_err) && empty($priape_err) && empty($num_err) && empty($address_err) && empty($loc_err) && empty($tipo_err)){
        $sql = "UPDATE usuario SET Email_Usu = ?, Nom_Usu = ?, PriApe_Usu = ?, SegApe_Usu = ?, Cel_Tel_Usu = ?, Direccion_Usu = ?, ID_Loc_Usu = ?, Tipo_Usu = ? WHERE ID_Usu = ?";
		
        if($stmt = mysqli_prepare($mysqli, $sql)){
			mysqli_stmt_bind_param($stmt, "ssssssssi", $param_email, $param_name, $param_priape, $param_segape, $param_num, $param_address, $param_loc, $param_tipo, $param_id);
					...
					<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
						...
						<div class="form-row">
							...
							<!--<div class="form-group col-md-4 <?php echo (!empty($pass_err)) ? 'has-error' : ''; ?>">
								...
								<input class='form-control' placeholder='CiscoClass' type='text' name="password" id="password" value="<?php //echo $pass; ?>">
							</div>-->
						...
						<div class="form-row">
							...
							<div class="form-group col-md-4">
								<input type="hidden" name="ID_Usu" value="<?php echo $id; ?>"/>
